class TravelPattern < ApplicationRecord
  ...
  def to_calendar(start_date, end_date = nil, valid_from = nil, valid_until = nil)
    ...
    # 2) find end_date by counting only days with actual service slots (excluding holidays and empty days)
    service_day_count = 0
    date_iter        = start_date
    hard_end         = end_date  # if provided, serves as an absolute cutoff

    while service_day_count < max_days && (hard_end.nil? || date_iter <= hard_end)
      # detect holiday
      ...
      unless has_holiday
        # see if any real slots exist on this date
        ...
        service_day_count += 1 if slots.any?
      end
    ...
    Rails.logger.info "[to_calendar] Pattern##{id} booking_window.maximum_days_notice=#{max_days} → capping after #{service_day_count} service days to end_date=#{end_date}"

    # 3) build the calendar only up to that service-based end_date
    calendar = {}
    date     = start_date
    ...
    while date <= end_date
      ...
      unless has_holiday
        ...
        # Map to start_time and end_time
  ...
  def self.to_api_response(travel_patterns, service, valid_from = nil, valid_until = nil)
    ...
    travel_patterns.map { |travel_pattern|
      ...
      Rails.logger.info "Initial start_date: #{date}"
      Rails.logger.info "Initial end_date: #{raw_end}"
      ...
      Rails.logger.info "Before while loop for end_date calculation: date: #{start_date}, days_notice: #{days_notice}, end_date: #{raw_end}, business_days: #{business_days}"
      ...
      while (days_notice < booking_window.maximum_days_notice && date_iter < raw_end) do
        date_iter += 1.day
        if pattern_calendar[date_iter.strftime('%Y-%m-%d')].any?
          days_notice += 1
        end
        Rails.logger.info "Inside while loop: date: #{date_iter}, days_notice: #{days_notice}, end_date: #{raw_end}"
      end

      Rails.logger.info "After while loop for end_date calculation: date: #{date_iter}, days_notice: #{days_notice}, end_date: #{raw_end}"
