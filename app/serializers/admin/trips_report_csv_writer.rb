module Admin
  class TripsReportCSVWriter < CSVWriter
    ...
    def booking_id
      @record.booking&.confirmation || 'No Booking ID'
    end

    def booking_client_id
      if @record.booking&.details&.dig(:client_id)
    ...
    def booking_timestamp
      @record.booking&.created_at&.strftime("%Y-%m-%d %H:%M:%S") || 'No Booking Timestamp'
    end

    def funding_source
      @record.booking&.details&.dig(:funding_hash, :funding_source) || 'No Funding Source'
    end

    def sponsor
      @record.booking&.details&.dig(:funding_hash, :sponsor) || 'No Sponsor'
    end

    def companions
      @record.booking&.itinerary&.companions || '0'
    end

    def trip_note
      @record.booking&.itinerary&.note || ' '
    end
    ...
    def is_round_trip
      @record.previous_trip_id.present? ? 'TRUE' : 'FALSE'
    end
  end
end