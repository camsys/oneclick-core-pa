  ...
  def trips_table
    ...
    if Config.dashboard_mode.to_sym == :travel_patterns && params[:ecolane_denied_trips_only].to_bool
      ...
      @trips = @trips.joins(:ecolane_booking_snapshot)
                     ...
                     .order(:trip_time)
      Rails.logger.info "Total matching trips for denied snapshots: #{@trips.count}"
      trip_ids = @trips.pluck(:id)
      # Directly query snapshots with denied disposition. Do not apply negotiated_pu filters here.
      snapshots = EcolaneBookingSnapshot.where(trip_id: trip_ids)
                                        .where(disposition_status: "Ecolane booking denial")
      Rails.logger.info "Total denied snapshots: #{snapshots.count}"
    else
      ...
      Rails.logger.info "Total snapshots in normal branch: #{snapshots.count}"
    end
