class Admin::ReportsController < Admin::AdminController
  ...
  def trips_table
    @snapshots = EcolaneBookingSnapshot.limit(CSVWriter::DEFAULT_RECORD_LIMIT)
    
    if @trip_time_from_date.present?
      @snapshots = @snapshots.where("negotiated_pu >= ?", @trip_time_from_date)
    end
    if @trip_time_to_date.present?
      @snapshots = @snapshots.where("negotiated_pu <= ?", @trip_time_to_date)
    end
    ...
    if Config.dashboard_mode.to_sym == :travel_patterns && params[:ecolane_denied_trips_only].to_bool
      @snapshots = @snapshots.where(disposition_status: Trip::DISPOSITION_STATUSES[:ecolane_denied])
    end
